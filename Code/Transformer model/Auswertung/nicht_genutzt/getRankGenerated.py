...
def createCSVFile(finallist, overallAvgScore):
    ...
    df.to_csv(config['PATHS']['getIndexGenerated'], sep='\t', encoding='utf-8')
...
def ZeroShotChainResultToFile(finallist):
    zeroShotResults = getJsonZeroshot(config['PATHS']['ZeroShotGeneratedEmoDefinition']) 
    zeroShotResults2 = getJsonZeroshot(config['PATHS']['ZeroShotGeneratedNeuDefinition'])
    finalzeroshot = []
...
zero_shot_results = open(config['PATHS']['Generated'])
